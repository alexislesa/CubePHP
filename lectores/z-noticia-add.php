...
/* Web titulo */
$web_titulo = "Nueva Noticia - ".$web_titulo;
$web_descripcion = "";

$pagina_actual = "lector-online";
$pagina_actual_2 = "noticiaadd";

/* Breadcrums */
$breds["Mi cuenta"] = "/lectores";
$breds["Publicar Noticia"] = "";
...
$include_file 	= dirTemplate."/lectores/noticia-add.inc.php";
$include_file_fin 	= dirTemplate."/lectores/noticia-add-fin.inc.php";
...
$checkForm[] = "required,terminosycondiciones,Debe aceptar los términos y condiciones";
...
if (!empty($_GET["act"])) {
	...
	if ($usrcheck->process()) {
		...
		if (!$error_msj) {
			...
			if (is_array($item_file) && count($item_file)>0) {
			...
			if (!$error_found) {
				...
				if (isset($item_file) && is_array($item_file) && count($item_file)) { 
			...
			if (!$error_found) {
				...
				/**
				 */
				// Extra, le cargo el estado desactivado para evitar que esa nota sea visible sin aprobación
				$_POST["estado"] = 0;
				$_POST["tipo"] = 7;
				...
				if (!$nota->save($_POST)) {
					...
					/**
					 * Agregado que me carga un punto extra por cada comentario realizado
					 * Cargo 2 puntos por cada nota
					 *
					 * Deshabilitado porque solo suma cuando estan aprobados
					 *
						$sql = "UPDATE lectores SET lector_puntaje = (lector_puntaje+2) WHERE lector_id = '{$usr->campos["lector_id"]}'";
						if (!$res = $db->query($sql)) {
							$error_msj = "Error al intentar actualizar información de su perfil.";
							$error_found = true;
						}
					 *
					 */
