...
$incBody = dirTemplate.'/'.$pathRelative.'/recupera-clave2.inc.php';
$incBodyEnd = dirTemplate.'/'.$pathRelative.'/recupera-clave2-fin.inc.php';
$incBar = dirTemplate.'/'.$pathRelative.'/barra.inc.php';


// Body class / Menú active
$pagActualClass[] = "lector-offline";
$pagActualClass[] = "recuperaclave";
...
$checkForm[] = "required, codigo, Ingrese el código enviaso a su casilla de email";
$checkForm[] = "length=0-10, codigo, El código ingresado es demasiado largo. Verifique si es correcto";
...
if (!empty($_GET['act'])) {
	...
	if ($usrCheck->process()) {
		...
		if ($clave = $usr->recoveryPassword($usrCheck->campos['email'],$usrCheck->campos['codigo'])) {
			...
			$email->email_asunto = "Envío de nueva clave de acceso desde ".$ss_config["site_name"];
			$email->destinatario = $email_config["to"];
			$email->datos = $campos;
			
			if ($email->process(dirTemplate."/emails/recupera_clave_2.htm")) {
